...
convertBtnEl.addEventListener("click", function () {
  ...
  const valueInFeetFromMeter = (number * 3.281).toFixed(3);
  const valueInMeterFromFeet = (number / 3.281).toFixed(3);

  const valueInLiterFromGallon = (number * 0.264).toFixed(3);
  const valueInGallonFromLiter = (number / 0.264).toFixed(3);

  const valueInKiloFromPound = (number * 2.204).toFixed(3);
  const valueInPoundFromKilo = (number / 2.204).toFixed(3);
